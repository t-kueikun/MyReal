...
## 特徴
- Stable Diffusion 3.5 画像生成（Stability API）+ 失敗時フォールバック
...
### Stable Diffusion 画像生成（推奨）
- `STABILITY_API_KEY`
- `STABLE_DIFFUSION_MODEL` (例: `sd3.5-large`)
- `STABLE_DIFFUSION_STRENGTH` (例: `0.92`)
- `STABLE_DIFFUSION_CFG_SCALE` (例: `6`)
- `STABLE_DIFFUSION_STYLE_PRESET` (例: `3d-model`)

`STABILITY_API_KEY` が設定されている場合は Stable Diffusion を最優先で使用します。
...
`OPENROUTER_API_KEY` は Stable Diffusion 失敗時のフォールバックとして利用されます。未設定なら直接Geminiを使用します。
